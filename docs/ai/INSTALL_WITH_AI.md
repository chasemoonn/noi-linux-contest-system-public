...
这份指令用于让 AI 帮助管理员理解环境、整理配置和解释只读诊断。它不是“让 AI 自动接管服务器”的脚本。当前 Alpha 只开放 `noictl` 的只读能力；安装、升级和回滚的事务化 `--apply` 在实现并通过演练前，仍必须由熟悉 Linux、Hydro 和 Docker 的管理员按部署文档执行。
...
7. 当只读门禁全部通过后，由管理员审阅部署计划。当前版本没有安全的通用 `install --apply`，AI 必须在这里停止。
...
7. 当前 Alpha 没有通用事务化 install --apply。只读诊断通过后，请输出一份人工审阅清单并停止，不得自行发明部署命令。
