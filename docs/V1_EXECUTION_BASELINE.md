...
1. 老师可选择 `web/folder/both`，存在不确定优先级；现行 V1 固定为逐题网页优先、整场无网页记录时目录兜底。
2. 开赛后新增学生需要老师显式批准；V1 以 OJ 报名为批准并自动扩座。
3. 座位池使用人工登记的正式上限与备用数；V1 默认按报名人数和 10%（至少 2 个）备用自动收敛。
4. 学生网页可粘贴代码；现行 V1 禁用粘贴，只允许选择完整 `.cpp` 文件。
5. 旧 `both` 模式的截止行为不明确；现行 V1 一旦某题有网页递交，就永不允许目录覆盖，未网页递交的题目才在截止使用目录。
