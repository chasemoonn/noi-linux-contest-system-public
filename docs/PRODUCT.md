...
- 固定的 `考号/题目名/题目名.cpp` 正式工作目录和一键开始答题入口；
- 与北京赛制一致的网页 `.cpp` 选择递交，以及其他地区可用的目录截止回收；
...
V1 不再让老师选择 `web`、`folder` 或 `both`，所有比赛使用同一固定规则：按“学生 × 题目”判断来源。只要该题在比赛中有过网页递交，就以截止前最后一次网页版本为准；只有整场从未网页递交的题目，截止时才从正式目录自动递交。目录版本永远不能覆盖已经存在的网页版本。
...
- 同一题可以多次网页递交，OJ 保留每次记录；最终采用截止前最后一次已确认的网页版本，而不是最高分版本。
- 某题没有任何网页递交时，截止冻结后才从正式目录读取 `题目名.cpp` 并创建一次目录兜底提交。
