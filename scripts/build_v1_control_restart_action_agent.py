#!/usr/bin/env python3
"""Freeze one qualification-only controller restart action agent."""
...
from v1_control_restart_action_agent import AgentError, validate_config

ROOT = Path(__file__).resolve().parents[1]
TEMPLATE = ROOT / "scripts" / "v1_control_restart_action_agent.py"
...
def safe_path(path: Path, *, file: bool):
    ...
    if requested != path or requested != resolved: raise BuildError("control restart agent path must be canonical and absolute")
    if platform.system().lower() == "linux":
        ...
        for part in resolved.parts[1:]:
            ...
            if not good or info.st_uid != 0 or stat.S_IMODE(info.st_mode) & 0o022:
                raise BuildError("control restart agent path metadata is unsafe")
    return resolved
...
def read_config(path: Path):
    ...
    try:
        ...
        if not 0 < info.st_size <= 1024 * 1024 or (platform.system().lower() == "linux" and
            ...
            raise BuildError("control restart config metadata is unsafe")
        raw = os.read(descriptor, info.st_size + 1)
        if len(raw) != info.st_size: raise BuildError("control restart config changed while reading")
    finally: os.close(descriptor)
    try: return validate_config(json.loads(raw.decode()))
    except (UnicodeDecodeError, json.JSONDecodeError, AgentError) as exc:
        raise BuildError("control restart config is invalid") from exc


def render(config):
    if platform.system().lower() == "linux": safe_path(TEMPLATE, file=True)
    source = TEMPLATE.read_text(encoding="utf-8")
    if source.count(MARKER) != 1: raise BuildError("control restart agent template marker differs")
    rendered = source.replace(MARKER, "EMBEDDED_CONFIG = " + pprint.pformat(config, sort_dicts=True))
    compile(rendered, "<v1-control-restart-action-agent>", "exec"); return rendered.encode()


def publish(path: Path, raw: bytes):
    requested = Path(os.path.abspath(path))
    if os.path.lexists(requested): raise BuildError("control restart agent output already exists")
    parent = safe_path(requested.parent, file=False)
    if platform.system().lower() == "linux" and stat.S_IMODE(parent.stat().st_mode) & 0o077:
        raise BuildError("control restart output parent is unsafe")
    descriptor, name = tempfile.mkstemp(prefix=".control-restart-agent-", dir=parent); temporary = Path(name)
    try:
...
def main():
    ...
    try:
        if platform.system().lower() != "linux" or os.geteuid() != 0: raise BuildError("control restart agent build requires Linux root")
