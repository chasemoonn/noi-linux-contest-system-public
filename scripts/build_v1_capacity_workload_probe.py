#!/usr/bin/env python3
"""Freeze one site's workload evidence configuration into one root-only probe."""
...
from v1_capacity_workload_probe import WorkloadProbeError, validate_config


ROOT = Path(__file__).resolve().parents[1]
TEMPLATE = ROOT / "scripts" / "v1_capacity_workload_probe.py"
...
def safe_path(path: Path, *, file: bool) -> Path:
    ...
    if requested != path or requested != resolved:
        raise BuildError("workload probe path must be canonical and absolute")
    if platform.system().lower() == "linux":
        ...
        for part in resolved.parts[1:]:
            ...
            if not good or info.st_uid != 0 or stat.S_IMODE(info.st_mode) & 0o022:
                raise BuildError("workload probe path metadata is unsafe")
    return resolved
...
def read_config(path: Path) -> dict:
    ...
    try:
        ...
        if not 0 < info.st_size <= 1024 * 1024 or (platform.system().lower() == "linux" and
            ...
            raise BuildError("workload probe config metadata is unsafe")
        raw = os.read(descriptor, info.st_size + 1)
        if len(raw) != info.st_size:
            raise BuildError("workload probe config changed while reading")
    finally:
        os.close(descriptor)
    try:
        return validate_config(json.loads(raw.decode()))
    except (UnicodeDecodeError, json.JSONDecodeError, WorkloadProbeError) as exc:
        raise BuildError("workload probe config is invalid") from exc
...
def render(config: dict) -> bytes:
    ...
    if source.count(MARKER) != 1:
        raise BuildError("workload probe template marker differs")
    rendered = source.replace(MARKER, "EMBEDDED_CONFIG = " + pprint.pformat(config, sort_dicts=True))
    compile(rendered, "<v1-capacity-workload-probe>", "exec")
    return rendered.encode()


def publish(path: Path, raw: bytes) -> str:
    requested = Path(os.path.abspath(path))
    if os.path.lexists(requested): raise BuildError("workload probe output already exists")
    parent = safe_path(requested.parent, file=False)
    if platform.system().lower() == "linux" and stat.S_IMODE(parent.stat().st_mode) & 0o077:
        raise BuildError("workload probe output parent is unsafe")
    descriptor, name = tempfile.mkstemp(prefix=".workload-probe-", dir=parent); temporary = Path(name)
    try:
...
def main() -> int:
    ...
    try:
        if platform.system().lower() != "linux" or os.geteuid() != 0:
            raise BuildError("workload probe build requires Linux root")
        digest = publish(args.output, render(read_config(args.config)))
        print(json.dumps({"probe_sha256": digest, "status": "built"}, sort_keys=True)); return 0
