#!/usr/bin/env python3
"""Freeze one qualification workload action agent configuration."""
...
from v1_capacity_workload_action_agent import ActionAgentError, validate_config


ROOT = Path(__file__).resolve().parents[1]
TEMPLATE = ROOT / "scripts" / "v1_capacity_workload_action_agent.py"
...
def safe_path(path: Path, *, file: bool) -> Path:
    ...
    if requested != path or requested != resolved: raise BuildError("workload action agent path must be canonical and absolute")
    if platform.system().lower() == "linux":
        current = Path(resolved.anchor)
        for part in resolved.parts[1:]:
            current /= part; info = current.lstat(); leaf = current == resolved
            good = stat.S_ISREG(info.st_mode) and info.st_nlink == 1 if leaf and file else \
            ...
            if not good or info.st_uid != 0 or stat.S_IMODE(info.st_mode) & 0o022:
                raise BuildError("workload action agent path metadata is unsafe")
    return resolved
...
def read_config(path: Path) -> dict:
    ...
    try:
        ...
        if not 0 < info.st_size <= 1024 * 1024 or (platform.system().lower() == "linux" and
            ...
            raise BuildError("workload action agent config metadata is unsafe")
        raw = os.read(descriptor, info.st_size + 1)
        if len(raw) != info.st_size: raise BuildError("workload action agent config changed while reading")
    finally: os.close(descriptor)
    try: return validate_config(json.loads(raw.decode()))
    except (UnicodeDecodeError, json.JSONDecodeError, ActionAgentError) as exc:
        raise BuildError("workload action agent config is invalid") from exc


def render(config: dict) -> bytes:
    if platform.system().lower() == "linux": safe_path(TEMPLATE, file=True)
    source = TEMPLATE.read_text(encoding="utf-8")
    if source.count(MARKER) != 1: raise BuildError("workload action agent template marker differs")
    rendered = source.replace(MARKER, "EMBEDDED_CONFIG = " + pprint.pformat(config, sort_dicts=True))
    compile(rendered, "<v1-capacity-workload-action-agent>", "exec"); return rendered.encode()


def publish(path: Path, raw: bytes) -> str:
    requested = Path(os.path.abspath(path))
    if os.path.lexists(requested): raise BuildError("workload action agent output already exists")
    parent = safe_path(requested.parent, file=False)
    if platform.system().lower() == "linux" and stat.S_IMODE(parent.stat().st_mode) & 0o077:
        raise BuildError("workload action agent output parent is unsafe")
    descriptor, name = tempfile.mkstemp(prefix=".workload-action-agent-", dir=parent); temporary = Path(name)
    try:
...
def main() -> int:
    ...
    try:
        if platform.system().lower() != "linux" or os.geteuid() != 0:
            raise BuildError("workload action agent build requires Linux root")
